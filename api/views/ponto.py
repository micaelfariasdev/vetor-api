...
def processar_colaborador(colaborador, ini, fim, mes_ponto):
    pontos_dic = {}
    mes_completo = gerar_datas_no_intervalo(ini, fim)
    for dia in mes_completo:
        pontos_dic[dia.isoformat()] = [formatar_semana(dia)]

    pontos_qs = colaborador.pontos.filter(data__range=[ini, fim])
    if not pontos_qs.exists():
        return None, HttpResponse("ponto não encontrado.", status=200)
    ...
    pontos = list(pontos_qs.values())
    feriados = [p['data'] for p in pontos if p['feriado']]
    ...
    for feriado in feriados:
        if formatar_semana(feriado) == "SÁB":
            ...
            for p in pontos:
                if data_inicio <= p['data'] <= data_fim:
                    p['sem_feriado'] = True

    for p in pontos:
        dia_semana = formatar_semana(p['data'])
        dataPonto = [
            dia_semana,
            str(p['entrada_manha'])[:5],
            str(p['saida_manha'])[:5],
            str(p['entrada_tarde'])[:5],
            str(p['saida_tarde'])[:5],
            p['horas_trabalhadas']
        ]

        for key in ['feriado', 'atestado', 'falta', 'ferias', 'sem_feriado']:
            if key in p and p[key]:
                dataPonto.append(key)

        if p.get('atestado') or p.get('ferias'):
            continue
        elif p.get('falta') and p['data'].month == mes_ponto.mes:
            falta += 1
            continue
        elif p.get('falta'):
            continue

        h, m = map(int, p['horas_trabalhadas'].split(':'))
        ...
        if 'sem_feriado' in p:
            ...
            if horas_trab < jornada_diaria:
                ...
                dif_hora = horas_trab - jornada_diaria
        elif dia_semana == "DOM" or p.get('feriado'):
            hr_fer += horas_trab
        elif dia_semana == "SEX":
            jornada_diaria = timedelta(hours=8)
            if horas_trab <= jornada_diaria:
                hr_falt += jornada_diab - horas_trab
                dif_hora = horas_trab - jornada_diaria
            else:
                hr_ext += horas_trab - jornada_diaria
                dif_hora = horas_trab - jornada_diaria
        ...
        pontos_dic[p['data'].isoformat()] = dataPonto

    dados = ColaboradorSerializer(colaborador).data
    dados.update({
        'horas-faltando': formatar_horas(hr_falt),
        'horas-extras': formatar_horas(hr_ext),
        'horas-feriado-domingo': formatar_horas(hr_fer),
        'falta': falta,
        'mes': mes_ponto.mes,
        'ano': mes_ponto.ano
    })

    return {'dados': dados, 'pontos': pontos_dic}, None
...
def pdf_pontos_relatorio(request, mes_id, col=None):
    ...
    try:
        ...
        if col:
            ...
            data, erro = processar_colaborador(colaborador, ini, fim, mes_ponto)
            if erro:
                return erro
            resultado.append(data)
        else:
            for colaborador in mes_ponto.obra.colaboradores.all():
                data, erro = processar_colaborador(colaborador, ini, fim, mes_ponto)
                if erro:
                    continue
                resultado.append(data)
